...
(with-test-dir
 ...
 (lambda (d)
   (let* ((project-dir "notes")
     ...
     (test-end)
     (test-begin "git fetch using a fetcher script that's plugged to mcron")
     (let ((fetcher-file ((@@ (x-files services pman)
                              fetcher-program-file) %manager)))
       (display (g:build fetcher-file))
       (test-assert (g:invoke (@@ (x-files services pman)
                                  fetcher-program-file)
                              %manager)))
     (test-end)
